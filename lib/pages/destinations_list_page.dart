...
class _DestinationsListPageState extends State<DestinationsListPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Panggilan ini juga menggunakan header Authorization
      final r = await DestinationService.list(perPage: 50, isActive: true);
      setState(() => _items = r);
    } catch (e) {
      // ignore: avoid_AppLogger.log
      AppLogger.log('DestinationsListPage: failed to load items: $e');
      setState(() => _items = []);
    } finally {
  ...
  Widget _emptyState() {
    if (kDebugMode) {
      return GridView.builder(
        ...
        itemCount: 6,
        itemBuilder: (context, i) {
          final img = 'https://picsum.photos/seed/dest$i/800/600';
          final title = 'Contoh Destinasi ${i + 1}';
          return ClipRRect(
            borderRadius: BorderRadius.circular(16),
            child: Container(
              decoration: BoxDecoration(
